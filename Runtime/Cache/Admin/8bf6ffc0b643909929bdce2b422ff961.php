  ...
  <div class="data-table table-striped">
    ...
    <tbody>
		<?php if(!empty($info)): if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('view?id='.$vo['id']);?>" class="confirm"><?php echo ($vo["title"]); ?></a></td>
			<td><?php echo (time_format($vo["create_time"])); ?></td>
			<td><?php if($vo["is_top"] == 0 ): ?>否 <?php else: ?> 是<?php endif; ?></td>
			<td>
				<?php if($vo["is_top"] == 0 ): ?><a class="ajax-get" href="<?php echo U('Notice/istop?status=1&id=' . $vo['id']);?>">置顶</a>
				<?php else: ?>
				  	<a class="ajax-get" href="<?php echo U('Notice/istop?status=0&id=' . $vo['id']);?>">取消置顶</a><?php endif; ?>
				<a href="<?php echo U('Edit?id='.$vo['id']);?>" class="confirm">编辑</a>
