...
<body>
    ...
    <div class="dynamic">
        <!--<?php if(($uid == $puid['uid'])): ?><div name="test" class="header-toolbar">
                <button class="btn post-dynamic" id="btn-link-post" data-href="<?php echo U('project/dynamiclistadd?id=_id');?>">发表动态</button>
                <button class="btn edit-dynamic" id="btn-link-edit" data-href="<?php echo U('project/dynamiclist/id/_id');?>">编辑动态</button>
            </div><?php endif; ?>-->
        <div class="dynamic_con">
            <ul>
                <?php if(is_array($dynamic)): foreach($dynamic as $key=>$vo): ?><li>
                        <em></em>
                        <div class="font">
                            <h1><?php echo (time_format($vo["create_time"],'Y-m-d H:i')); ?></h1>
                            <h2>
                                <a href="<?php echo U('project/projectdynamicdetail?id='.$vo['id']);?>">
                                    <?php echo ($vo["title"]); ?>
                                </a>
                            </h2>
