...
<span class="rtlink"><a href="<?php echo U('Project/create');?>" class="fq">+ 发起项目</a></span>
	...
	<div class="userright2 ri">
		<h2><?php echo ($myProduct["name"]); ?></h2>
      	<p class="ui-top-info">到账金额 : <?php echo ($listSum); ?> (不包含邮费)
            <a href="<?php echo U('Manage/exploadMyCrowdfundingOrder?pid='.$pid);?>" class="btn btn-primary ui-export-order">导出订单信息</a>
				...
				<table class="tt">
				    ...
				    <tbody>
				    	<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
							<td><?php echo ($k); ?></td>
							<td><a href="<?php echo U('Manage/orderinfo?id='.$vo['id']);?>"><?php echo ($vo["id"]); ?></a></td>
							<td><span>￥<?php echo ($vo['amount']); ?></span>
								<span>(邮费：￥<?php echo ($vo['post_amount']); ?>)</span>
							</td>
								<td><?php echo ($customAddress[$vo['address_id']]['name']); ?> <br/>
									<?php echo ($customAddress[$vo['address_id']]['phone']); ?></td>
								<td>
									<?php echo (getprovincecity($customAddress[$vo['address_id']]['province'],$customAddress[$vo['address_id']]['city'])); ?> <br><?php echo ($customAddress[$vo['address_id']]['address']); ?>
								</td>
								<td>
									<span><?php echo (time_format($vo['create_time'],'Y-m-d')); ?></span>
									<span><?php echo (time_format($vo['create_time'],'H:i:s')); ?></span>
								</td>
								<td><?php echo ($vo['luckno']); ?></td>
								<td>
									<!-- 状态 回报相关id（0:购买成功、1:支付成功、-1:撤消 -2没有购买名额了） -->
									<?php if($vo['status'] == 0): ?><span class="label">已购买</span>
										<?php elseif($vo['status'] == 1): ?><span class="label label-success">已支付</span>
										<?php elseif($vo['status'] == 2): ?><span class="label label-warning">支付成功金额不足</span>
										<?php elseif($vo['status'] == -1): ?><span class="label label-warning">已撤销</span>
										<?php elseif($vo['status'] == -2): ?><span class="label label-inverse">无购买名额</span><?php endif; ?>
								</td>
