...
<body>
    ...
    <div id="server-data" style="display: none;">
        <?php if($need_pay > 0): ?><input name="actionUrl" type="hidden" value="<?php echo U('Pay/quickpay1?merorderid='.$data['merorderid']);?>">
            <?php else: ?>
            <input name="actionUrl" type="hidden" value="<?php echo U('Pay/usebalance?merorderid='.$data['merorderid']);?>"><?php endif; ?>
    </div>

    <form class="pay-remaining" action="<?php echo U('querycard');?>" method="post">
        ...
        <input type="hidden" name="balance" value="<?php echo ($need_pay); ?>">

        <div class="tac panel-flat mt-8 mb-5">
            <div class="fz-30 c-blue">￥<span class="serif"><?php echo ($data["pay_amount"]); ?></span>元</div>
            <div>投资金额</div>
        </div>
        <?php if($account["use_able"] > 0): ?><div class="panel-flat inline-label-text fz-18 mb-5">
                <label class="">账户余额</label>
                <span class="input"><span class="serif mr-5"><?php echo ($account["use_able"]); ?></span>元</span>
            </div><?php endif; ?>
        <?php if($need_pay > 0): ?><div class="panel-flat inline-label-text fz-18 mb-10">
                <label class="">充值金额</label>
                <span class="input"><span class="serif mr-5"><?php echo ($need_pay); ?></span>元</span>
            </div><?php endif; ?>
