...
<ul>
  <li><a href="<?php echo U('Message/index');?>">我的私信</a></li>
  <li class="cur"><a href="<?php echo U('Message/system');?>">系统通知</a></li>
</ul>
...
<div class="userright2 ri">
  ...
  <?php if(is_array($messages)): $i = 0; $__LIST__ = $messages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><dl>
      <dd class="detail" style="width:650px;">
        <div class="d">系统消息 (<?php echo (time_format($vo["add_time"],"Y-m-d H:m:s")); ?>)</div>
        <div class="from"><b><?php echo ($vo["content"]); ?></b></div>
      </dd>
      <dd class="cz"><a href="<?php echo U('setStatus?Model=Message&type=-1&status=2&ids='.$vo['id']);?>" class="btn-delete">删除</a></dd>
