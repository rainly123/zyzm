...
<div class="userright2 ri">
  <div class="border1 zc clearfix" style="text-align:left">
    <div style="float:left;margin-left:30px; margin-top:5px;">
		已买入：<em><?php echo ((isset($data["sumfund"]) && ($data["sumfund"] !== ""))?($data["sumfund"]):0); ?></em><i>元</i>
		已赚取：<em <?php if($data["sumover"] > 0): ?>style="color:#f35d5d;" <?php else: ?>  style="color:#72c2e5;"<?php endif; ?>><?php echo ((isset($data["sumover"]) && ($data["sumover"] !== ""))?($data["sumover"]):0); ?></em><i>元</i>
		排队中：<em><?php echo ((isset($data["sumwaiting"]) && ($data["sumwaiting"] !== ""))?($data["sumwaiting"]):0); ?></em><i>元</i>
    </div>
  </div>
  <div class=" mt20">
    <div class="top">
    <table>
      ...
      <?php if(is_array($stockinfo)): $i = 0; $__LIST__ = $stockinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
    	<td width="120" class="img">
			<a href="<?php echo U('Stock/index?id='.$vo['sid']);?>"><img src="<?php echo (get_cover($vo["logo"],'path')); ?>"></a>
		</td>
	    <td width="200">
	    	<a href="<?php echo U('Stock/index?id='.$vo['sid']);?>"><?php echo ($vo["name"]); ?></a>
	    	<em class="z">单位净值:<?php echo ($vo["assets"]); ?></em>
	    </td>
        <td>¥<?php echo round($vo['fund']);?></td>
        <td>¥<?php echo round($vo['amount'],2);?></td>
        <td><?php echo ($vo["operation_fund"]); ?></td>
        <td <?php if($vo["over"] > 0): ?>style="color:#f35d5d;" <?php else: ?>  style="color:#72c2e5;"<?php endif; ?> >¥<?php echo ($vo["over"]); ?></td>
        <td>¥<?php echo round($vo['waiting_fund']);?></td>
        <td class="cz"><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Stock/index?id='.$vo['sid']);?>">去投资</a><?php endif; ?>
...
// 指定左侧选中菜单
var submenu = "<?php echo U('MCenter/stock');?>";
